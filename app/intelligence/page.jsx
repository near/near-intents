...
const TH=({c})=><th style={{textAlign:"left",padding:"9px 14px",color:T.textDim,fontFamily:T.mono,fontSize:9,fontWeight:700,letterSpacing:"0.1em",textTransform:"uppercase",borderBottom:`0.5px solid ${T.borderCard}`,whiteSpace:"nowrap"}}>{c}</th>;
const TD=({children,s={}})=><td style={{padding:"10px 14px",borderBottom:`0.5px solid ${T.borderSubtle}`,verticalAlign:"middle",...s}}>{children}</td>;
  ...
  return(<>
    ...
    <div style={{minHeight:"100vh",background:T.bgBase,color:T.textPrimary,fontFamily:T.display}}>
      ...
      <div style={{background:T.bgSurface,borderBottom:`0.5px solid ${T.borderCard}`,position:"sticky",top:0,zIndex:100}}>
        <div style={{display:"flex",justifyContent:"space-between",padding:"4px 40px",fontFamily:T.mono,fontSize:10,color:T.green,opacity:0.2,borderBottom:`0.5px solid ${T.borderSubtle}`}}>{Array.from({length:20},(_,i)=><span key={i}>+</span>)}</div>
        <div style={{display:"flex",alignItems:"stretch",padding:"0 40px",height:52}}>
          <div style={{display:"flex",alignItems:"center",gap:12,paddingRight:28,marginRight:4,borderRight:`0.5px solid ${T.borderCard}`}}>
            <NearLogo/>
            <div>
      ...
      <div style={{padding:"32px 40px",maxWidth:1480,margin:"0 auto"}}>
        ...
        {nav==="Overview"&&<>
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:32}}>
            <div>
              ...
              <div style={{fontFamily:T.display,fontSize:44,fontWeight:800,color:T.textPrimary,letterSpacing:"-0.025em",lineHeight:1.05,marginBottom:12}}>KOL + Fund Manager<br/>Intelligence Platform</div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"0.75fr 1.3fr 1.3fr",gap:10,marginBottom:10}}>
        ...
        {nav==="KOL Intel"&&<>
          ...
          <div style={{display:"flex",alignItems:"flex-end",justifyContent:"space-between",marginBottom:24}}>
            <div><PL>Trader KOL Intelligence</PL><div style={{fontFamily:T.display,fontSize:36,fontWeight:800,color:T.textPrimary,letterSpacing:"-0.02em"}}>{fKOLs.length} KOLs</div></div>
          ...
          <Card>
            <table style={{width:"100%",borderCollapse:"collapse"}}>
              <thead><tr>{["#","ACCOUNT","TIER","TYPE","TAGS","FOLLOWERS","◈ NEAR","LIKES","SCORE"].map(c=><TH key={c} c={c}/>)}</tr></thead>
              <tbody>{fKOLs.map((k,i)=>(<tr key={k.Handle+i} className="rh">
                ...
                <TD s={{fontFamily:T.mono,fontSize:10,color:T.textSecondary,maxWidth:140}}>{k.Type||"—"}</TD>
                <TD><div style={{display:"flex",gap:4,flexWrap:"wrap"}}>{k.Tags.slice(0,3).map(t=><Tag key={t}>{t}</Tag>)}</div></TD>
                ...
                <TD s={{fontFamily:T.mono,fontSize:11,color:T.textMuted}}>{fmt(k["Total Likes"]||0)}</TD>
        ...
        {nav==="Fund Intel"&&<>
          <PlusRow/>
          <div style={{display:"flex",alignItems:"flex-end",justifyContent:"space-between",marginBottom:24}}>
            <div><PL>Fund Manager Intelligence</PL><div style={{fontFamily:T.display,fontSize:36,fontWeight:800,color:T.textPrimary,letterSpacing:"-0.02em"}}>{fFunds.length} Fund Managers</div></div>
          ...
          <Card>
            <table style={{width:"100%",borderCollapse:"collapse"}}>
              <thead><tr>{["#","MANAGER","FUND","ROLE","CATEGORIES","FOLLOWERS","ENGAGEMENT","SCORE"].map(c=><TH key={c} c={c}/>)}</tr></thead>
              <tbody>{fFunds.map((f,i)=>(<tr key={f.Handle+i} className="rh">
                ...
                <TD s={{fontFamily:T.display,fontSize:11,color:T.textMuted,maxWidth:140}}>{f.Role||"—"}</TD>
                <TD><div style={{display:"flex",gap:4,flexWrap:"wrap"}}>{(f.tags||[]).slice(0,2).map(t=><Tag key={t}>{t}</Tag>)}</div></TD>
        ...
        {nav==="NEAR Tweets"&&<>
          <PlusRow/>
          <div style={{display:"flex",alignItems:"flex-end",justifyContent:"space-between",marginBottom:24}}>
            <div><PL>NEAR Mention Stream</PL><div style={{fontFamily:T.display,fontSize:36,fontWeight:800,color:T.textPrimary,letterSpacing:"-0.02em"}}>{fTweets.length} Tweets</div></div>
        ...
        {nav==="Cross-Map"&&<>
          ...
          <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:24}}>
            <div><PL>Cross-Ecosystem Intelligence</PL><div style={{fontFamily:T.display,fontSize:36,fontWeight:800,color:T.textPrimary,letterSpacing:"-0.02em"}}>KOL × Fund × Tag Overlap</div></div>
            <BetNum n="05" label="Cross-Map"/>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"1.5fr 1fr",gap:10}}>
            <Card><SL>◈ NEAR-Active KOL Leaderboard</SL>
              {nearKOLs.map((k,i)=>(<div key={k.Handle+i} style={{display:"grid",gridTemplateColumns:"24px 28px 1fr 70px 60px",gap:10,alignItems:"center",padding:"9px 0",borderBottom:`0.5px solid ${T.borderSubtle}`}}>
                ...
                <div style={{fontFamily:T.mono,fontSize:8,fontWeight:700,textTransform:"uppercase",color:TIER_C[k.tier],textAlign:"right"}}>{k.tier}</div>
              </div>))}
            </Card>
        ...
        {nav==="Strategy"&&<>
          <PlusRow/>
          <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:24}}>
            <div><PL>Strategic Recommendations</PL><div style={{fontFamily:T.display,fontSize:36,fontWeight:800,color:T.textPrimary,letterSpacing:"-0.02em"}}>NEAR Ecosystem Growth Pipeline</div></div>
      ...
      <div style={{borderTop:`0.5px solid ${T.borderSubtle}`,padding:"12px 40px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
