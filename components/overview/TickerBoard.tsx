        ...
        {displayTokens.map((token, i) => {
          ...
          return (
            <div key={`${token.symbol}-${i}`} className="relative">
              <button
                className={`flex w-full items-center gap-2.5 rounded-lg border px-3 py-2.5 text-left shadow-sm transition-all sm:gap-3 sm:px-4 sm:py-3 ${
                  isActive
                    ? 'bg-[#2e2e2e] border-white/20 shadow-md z-20'
                    : 'bg-[#242424] border-white/10 hover:bg-[#2e2e2e] hover:border-white/20'
                }`}
